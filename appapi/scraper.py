...
connection = sqlite3.connect("appapi/db.sqlite3")
...
def aujourdui_scrape_articles(press_name):
    """
    Extraction des liens des articles et insertion dans la base de données.
    Args:
        press_name: Le nom de la presse pour chaque appel d'API
    """
    liens_articles = []
    for page in range(1):
    ...
    for lien in liens_articles:
        ...
        if contenu:
            ...
            # Combine the title and description into the article content
            article = titre + " " + description if titre and description else titre or description
            row = [article, date, press_name]
            
            # Insert data into SQLite database using parameterized query
            cursor.execute("""
                INSERT INTO article_table (article, date, press_name)
                VALUES (?, ?, ?)
            """, (article, date, press_name))
